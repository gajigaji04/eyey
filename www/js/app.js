(() => {
  ...
  let schedule = [];
  let completions = {};
  let breakTimer = { enabled: false, startedAt: null };
  let lastBreakPhase = null; // for edge-detecting phase flips on the web tick loop
  ...
  const el = {
    ...
    breakContent: document.getElementById('break-content'),
  };
  ...
  function recompute() {
    ...
    // Interval doses rebase off actual completion times, so completions must
    // be known before the schedule is built.
    schedule = buildTodaySchedule(medications, pattern, completions);
    breakTimer = getBreakTimer();
  }
  ...
  function renderToday() {
    ...
    el.scheduleList.innerHTML = schedule
      .map((d) => {
        ...
        const timeField = isDone
          ? `<input type="time" class="dose-time-input" data-dose-id="${d.id}" value="${formatMinutesToTime(actualMinutes(completions[d.id]))}" title="실제로 넣은 시간 수정" />`
  ...
  function actualMinutes(timestamp) {
    const d = new Date(timestamp);
    return d.getHours() * 60 + d.getMinutes();
  }

  // Renders the break-timer card and, on the web, fires a notification the
  // instant the derived phase flips (native builds get real OS alarms instead
  // via Notify.scheduleBreakTimerNative).
  function renderBreak() {
    el.breakToggle.checked = breakTimer.enabled;
    if (!breakTimer.enabled) {
      el.breakContent.innerHTML = '<p class="muted">꺼져 있어요. 켜면 50분마다 쉬라고 알려드려요.</p>';
      lastBreakPhase = null;
      return;
    }
    const { phase, remainingSeconds } = computeBreakPhase(breakTimer.startedAt, Date.now());
    const label = phase === 'work' ? '화면 사용 중' : '휴식 중 👀';
    el.breakContent.innerHTML = `
      <div class="break-phase ${phase}">${label}</div>
      <div class="break-countdown">${formatDuration(remainingSeconds)} 후 ${phase === 'work' ? '휴식' : '재시작'}</div>`;

    if (lastBreakPhase && lastBreakPhase !== phase) {
      Notify.fireBreak(phase === 'rest' ? 'rest' : 'work');
    }
    lastBreakPhase = phase;
  }

  function toggleBreakTimer(enabled) {
    breakTimer = { enabled, startedAt: enabled ? Date.now() : null };
    saveBreakTimer(breakTimer);
    lastBreakPhase = null;
    renderBreak();
    if (enabled) Notify.scheduleBreakTimerNative(breakTimer.startedAt);
    else Notify.cancelBreakTimerNative();
  }
  ...
  function toggleDose(doseId, done) {
    ...
    recompute(); // completing an interval dose reschedules the ones after it
    renderToday();
    Notify.syncNativeSchedule(schedule, completions);
  }

  function editDoseTime(doseId, hhmm) {
    setCompletionTime(doseId, hhmm);
    recompute();
    renderToday();
    Notify.syncNativeSchedule(schedule, completions);
  }
  ...
  function onPatternChange() {
    pattern = {
      ...
      sleep: el.patternSleep.value || '23:59',
    };
    savePattern(pattern);
    recompute();
    renderToday();
  ...
  async function checkAndFireNotifications() {
    if (Notify.isNative()) return; // native OS alarms already deliver these
  ...
  function init() {
    recompute();
    if (handleCompleteFromUrl()) recompute();
    renderToday();
    renderMeds();
    renderSettings();
    renderBreak();
    ...
    el.scheduleList.addEventListener('change', (e) => {
      ...
      } else if (e.target.classList.contains('dose-time-input') && e.target.value) {
        editDoseTime(e.target.dataset.doseId, e.target.value);
      }
    });
    ...
    el.notifBtn.addEventListener('click', async () => {
      ...
      updateNotifButton();
      Notify.syncNativeSchedule(schedule, completions);
      if (breakTimer.enabled) Notify.scheduleBreakTimerNative(breakTimer.startedAt);
    });
    el.breakToggle.addEventListener('change', (e) => toggleBreakTimer(e.target.checked));
    ...
    // Fires when a native OS alarm (Capacitor LocalNotifications) is tapped.
    window.addEventListener('eyesalarm:complete-dose', (event) => {
      toggleDose(event.detail.doseId, true);
    });
